...
class PUNet(Network):
    ...
    def temp_loss_raw(self, y_true, y_pred, use_emd, acc_fac, old=False):
        ...
        if not old:
            ...
            return (1 - acc_fac) * (K.mean(K.abs(pred_v0 - gt_v0), axis=-1) + K.mean(K.abs(pred_v1 - gt_v1), axis=-1)) / 2 + acc_fac * K.mean(K.abs(pred_a - gt_a), axis=-1) + 10 * K.mean(K.abs(pred_v0 - K.expand_dims(K.mean(pred_v0, axis=1), axis=1)), axis=-1)
